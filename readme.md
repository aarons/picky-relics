...
- **Per-Tier Settings**: Configure different choice counts for each relic tier
- **Tier Change Algorithm**: Control the chance for additional relics to change tiers
- **Original Behavior**: Set to 1 to restore vanilla game behavior for any tier
- **Visual Labels**: Toggle tier labels on reward screens for clarity
...
Access the mod settings through the in-game mod config menu, organized in two columns:

### Choices Per Tier (Left Column)
Configure how many relic options appear for each tier:
- **Starter**: Number of choices for starter relics (1-5, default: 2)
- **Common**: Number of choices for common relics (1-5, default: 2)
- **Uncommon**: Number of choices for uncommon relics (1-5, default: 2)
- **Rare**: Number of choices for rare relics (1-5, default: 2)
- **Boss**: Number of choices for boss relics (1-5, default: 2)
- **Shop**: Number of choices for shop relics (1-5, default: 2)
- **Event**: Number of choices for event relics (1-5, default: 2)

Set any tier to 1 for original game behavior (no extra choices).

### Algorithms (Right Column)
- **Chance for picky relics to change tiers**: Controls tier modification for additional relics (-100% to +100%, default: 0%)
  - **-100%**: Always lowest tier in hierarchy
  - **0%**: Use original tier (no change)
  - **+100%**: Always highest tier in hierarchy

The tier change algorithm only affects additional relics generated by Picky Relics, not the original relic awarded by the game. Each additional relic rolls independently for tier changes.
...
### Display Options
- **Show relic tier labels**: Toggle tier labels on reward screens (default: enabled)
...
└── src/main/
    ...
    │   ├── patches/
    │   │   └── RelicLinkPatch.java           # Core patching and tier change logic
    │   └── util/
...
The mod uses SpirePatch to intercept relic rewards when they're displayed to the player. It creates linked relic rewards (the same mechanism used for boss relic choices) by:

1. **Preserving the original relic** that the game awarded
2. **Generating additional relics** based on the configured choice count for that tier
3. **Applying tier changes** to additional relics using the configured algorithm
4. **Linking all relics together** so claiming one removes the others

#### Tier Change Algorithm
When enabled, each additional relic independently rolls for tier changes:
1. Roll against the absolute percentage value (e.g., 30% chance for +30% setting)
2. If successful, change tier by one step in the appropriate direction
3. Continue rolling until a roll fails or the tier boundary is reached
4. Generate a relic from the final modified tier

Example with -30% setting starting from Rare:
- Roll 1: Success (within 30%) → Rare becomes Uncommon
- Roll 2: Failure (outside 30%) → Stays Uncommon
- Result: Uncommon relic is awarded
